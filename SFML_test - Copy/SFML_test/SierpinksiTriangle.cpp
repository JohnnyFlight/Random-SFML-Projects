...
SierpinskiTriangle::SierpinskiTriangle(unsigned depth)
{
	_values.push_back(std::vector<long long unsigned>(1, 1));
	_depth = 1;
...
long long unsigned SierpinskiTriangle::getValue(unsigned x, unsigned y)
{
	//	Return 0 if invalid
	if (y > x) return 0;
	while (y > _depth)
...
void SierpinskiTriangle::addRow()
{
